...
from vikopti.operators import cross


def test(n_var=2, method="sbx", random=False):
    ...
    # Generate 2 parents
    if random:
        xp1 = np.random.uniform(lower, upper)
        xp2 = np.random.uniform(lower, upper)
    else:
        xp1 = np.full(n_var, 0.5)
        xp2 = np.full(n_var, -0.5)
    ...
    for eta in etas:
        for _ in range(n_cross):
            xoff = cross(xp1, xp2, bounds, method=method, eta=eta)
